const { createWebdriverChrom, createWebdriverSafari } = require('../src/utils/webdriver');
...
describe('Create, edit and remove project by the SA and measure metrics', async () => {
  ...
  it('create new project Safari', async () => {
    // time and site or lochalhost there tests are going
    console.log(Date().toLocaleLowerCase(), 'date', config.urlLoginPage);

    const logginPageTest = new LoginPage(driverSafari, config.urlLoginPage);
    const createProjectTest = new CreateProject(driverSafari);
    await logginPageTest.openLoginForm();
    await logginPageTest.fillEmailInput(config.email);
    await logginPageTest.fillPasswordInput(config.password);
    await logginPageTest.checkSaveForFuture();
    ...
    try {
      ...
      await makeScreenshot(driverSafari, 'project_create');
      throw error;
    }
  });

  it('remove project Safari', async () => {
    // time and site or lochalhost there tests are going
    console.log(Date().toLocaleLowerCase(), 'date', config.urlLoginPage);

    const logginPageTest = new LoginPage(driverSafari, config.urlLoginPage);
    const removeProject = new RemoveProject(driverSafari);
    ...
    try {
      ...
      await makeScreenshot(driverSafari, 'project_remove');
      throw error;
    }
  });
});
