...
class SequenceUnitChange extends Base{
   ...
   async sequenceChange(){
        ...
        const numberOfItems = await this.driver.wait(until.elementsLocated(By.css('.unit-name-with-btn-wrapper')),10000).catch(()=>null);
    ...
    async checkSequence(){
        const afterChange = await this.driver.findElements(By.css('.unit-name-with-btn-wrapper'));
...
module.exports = SequenceUnitChange